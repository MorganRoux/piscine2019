...
/*   Updated: 2019/08/17 11:30:53 by mroux            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	next_item(t_list **lprev, t_list **lcurrent, t_list **lnext)
{
	*lprev = *lcurrent;
	*lcurrent = *lnext;
	*lnext = (*lcurrent)->next;
	...
	while (!end)
	{
		...
		while (lnext != 0)
		{
			if ((*cmp)(lcurrent->data, lnext->data) > 0)
			{
				...
				*begin_list = (lprev == 0) ? lnext : *begin_list;
				ft_list_swap(lprev, &lcurrent, &lnext);
			}
			next_item(&lprev, &lcurrent, &lnext);
		}
	}
}
